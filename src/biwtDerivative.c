...
SEXP diffCalBW(SEXP data, SEXP srow, SEXP scol, SEXP sh, SEXP smaxDist) {
  ...
  nrow = (int) NUMERIC_POINTER(srow)[0];
  ncol = (int) NUMERIC_POINTER(scol)[0];
  maxDist = (int) NUMERIC_POINTER(smaxDist)[0];
  h = NUMERIC_POINTER(sh)[0];
  cdata = NUMERIC_POINTER(data);
...
void createDiffTable(double *table, 
		     double h, int maxDist) {
  int i, j;
  for(i = 0; i <= maxDist; i++) { 
    for(j = 0; j <= maxDist; j++) {
